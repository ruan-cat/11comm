...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listParkingLots({
			pageIndex: Number(body.pageIndex ?? body.page ?? 1),
			pageSize: Number(body.pageSize ?? 20),
			parkingLotNumber: toOptionalTrimmedString(body.parkingLotNumber),
			parkingLotType: toOptionalTrimmedString(body.parkingLotType),
