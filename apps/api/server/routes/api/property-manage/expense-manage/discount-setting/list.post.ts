...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listDiscountSettings({
			page: Number(body.page || body.pageIndex || 1),
			pageIndex: Number(body.pageIndex || body.page || 1),
			pageSize: Number(body.pageSize || 20),
			applicableItem: toOptionalTrimmedString(body.applicableItem),
			discountType: toOptionalTrimmedString(body.discountType),
