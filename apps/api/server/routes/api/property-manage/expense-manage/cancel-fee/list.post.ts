...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listCancelFees({
			page: Number(body.page || body.pageIndex || 1),
			pageIndex: Number(body.pageIndex || body.page || 1),
			pageSize: Number(body.pageSize || 20),
			chargeId: toOptionalTrimmedString(body.chargeId),
			chargeType: toOptionalTrimmedString(body.chargeType),
			employee: toOptionalTrimmedString(body.employee),
			cancelReason: toOptionalTrimmedString(body.cancelReason),
