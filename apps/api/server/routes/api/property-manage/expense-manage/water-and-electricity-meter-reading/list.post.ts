...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listWaterAndElectricityMeterReading({
			page: Number(body.page || body.pageIndex || 1),
			pageIndex: Number(body.pageIndex || body.page || 1),
			pageSize: Number(body.pageSize || 10),
			meterId: toOptionalTrimmedString(body.meterId),
			meterType: toOptionalTrimmedString(body.meterType),
