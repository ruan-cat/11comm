...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listBuildingSpaceStructures({
			...
			buildingNo: toOptionalTrimmedString(body.buildingNo),
			floorCount: body.floorCount !== undefined ? Number(body.floorCount) : undefined,
			unitCount: body.unitCount !== undefined ? Number(body.unitCount) : undefined,
		});
	} catch (error) {
