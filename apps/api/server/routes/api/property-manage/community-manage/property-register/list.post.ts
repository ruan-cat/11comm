...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listPropertyRegisters({
			...
			registerType: toOptionalTrimmedString(body.registerType),
			applicant: toOptionalTrimmedString(body.applicant),
			status: toOptionalTrimmedString(body.status),
			propertyName: toOptionalTrimmedString(body.propertyName),
			propertyCode: toOptionalTrimmedString(body.propertyCode),
			registerDate: toOptionalTrimmedString(body.registerDate),
			remark: toOptionalTrimmedString(body.remark),
		});
	} catch (error) {
