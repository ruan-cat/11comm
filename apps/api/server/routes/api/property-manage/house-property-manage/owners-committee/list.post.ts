...
export default defineHandler(async (event) => {
	try {
		...
		return await adminAdapter.listOwnersCommittees({
			pageIndex: Number(body.pageIndex ?? body.page ?? 1),
			pageSize: Number(body.pageSize ?? 20),
			committeeName: toOptionalTrimmedString(body.committeeName),
			term: toOptionalTrimmedString(body.term),
			chairman: toOptionalTrimmedString(body.chairman),
