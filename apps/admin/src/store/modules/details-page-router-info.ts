...
/** 本地存储的 key */
const STORAGE_KEY = `${responsiveStorageNameSpace()}details-page-router-info`;

/**
 * 详情页路由信息存储 Store
 * @description 基于路由地址的键值对存储工具，使用组合式 API 实现
 */
export const useDetailsPageRouterInfoStore = defineStore("details-page-router-info", () => {
	// 状态：路由信息映射表
	...
	function getRouteInfo(key: RouteRecordName): RouteInfoData | null {
		...
		// 直接从本地存储获取数据
		const storedData = storageLocal().getItem<Record<string, RouteInfoData>>(STORAGE_KEY);
		return storedData?.[stringKey] || null;
	}
	...
	/**
	 * 清空所有路由信息
	 * TODO: 在退出登录 清空缓存 的地方，使用本函数实现信息清空
	 */
...
/**
 * 详情页路由信息存储 Hook 函数
 * @description 用于在组件中便捷使用详情页路由信息存储功能
 * @returns Store 实例
 */
export function useDetailsPageRouterInfoStoreHook() {
	return useDetailsPageRouterInfoStore(store);
}
