...
/** 页面接线测试通过读取源码校验正式 API hook、JSON payload 转换和弹窗提交链路。 */
...
const realFields = ["initItem", "initStatus", "configParams"];
const removedMockTokens = ["testAsync", "sleep(1300)", "consola", "useToggle"];
const forbiddenFormFields = ["communityId", "communityName", "nearbyLandmark", "cityCode", "status"];
...
describe("initialize-cell page api wiring", () => {
	test("page add/info/edit/delete actions must use real initialize-cell callers", () => {
		const source = readFileSync(pageSourcePath, "utf8");
		const viewDetailsBlock = extractFunctionBlock(source, "viewInitializeCellDetails");
		const editBlock = extractFunctionBlock(source, "editInitializeCell");
		const deleteBlock = extractAsyncFunctionBlock(source, "deleteInitializeCellRow");

		expect(source).toMatch(/openDialog\(\s*{\s*mode:\s*["']add["']/);

		expect(viewDetailsBlock).toMatch(/openDialog\(\s*{\s*mode:\s*["']info["']/);
		expect(viewDetailsBlock).not.toContain("createInitializeCell");
		expect(viewDetailsBlock).not.toContain("updateInitializeCell");
		expect(viewDetailsBlock).not.toContain("deleteInitializeCell(");

		expect(editBlock).toMatch(/openDialog\(\s*{\s*mode:\s*["']edit["']/);
		expect(editBlock).not.toContain("createInitializeCell");
		expect(editBlock).not.toContain("deleteInitializeCell(");

		expect(deleteBlock).toContain("ElMessageBox.confirm");
		expect(deleteBlock).toContain("await deleteInitializeCell");
		expect(deleteBlock).toContain("await doFetch()");
		expect(deleteBlock).not.toContain("createInitializeCell");
		expect(deleteBlock).not.toContain("updateInitializeCell");

		expect(source).toMatch(/<ElButton type="primary" @click="openDialog\({ mode: 'add' }\)">/);
		expect(source).toMatch(/<ElButton type="info" @click="viewInitializeCellDetails\(row\)">/);
		expect(source).toMatch(/<ElButton type="warning" @click="editInitializeCell\(row\)">/);
		expect(source).toMatch(/<ElButton type="danger" @click="deleteInitializeCellRow\(row\)">/);
	...
	test("dialog submit must call create/update and refresh list, while info mode does not submit", () => {
		...
		expect(submitButtonBlock).toMatch(/if\s*\(\s*isAdd\.value\s*\)\s*{\s*await createInitializeCell/);
		expect(submitButtonBlock).toMatch(
			/}\s*else if\s*\(\s*isEdit\.value\s*&&\s*row\?\.id\s*\)\s*{\s*await updateInitializeCell/,
		);
	...
	test("payload and form fields must match sm_initialize_cells real fields", () => {
		...
		expect(formSource).toContain("InitializeCellFormData");
		expect(pageSource).toContain("InitializeCellFormData");
		...
		for (const field of forbiddenFormFields) {
			expect(formSource).not.toContain(`${field}:`);
			expect(formVueSource).not.toContain(`prop: "${field}"`);
			expect(pageSource).not.toContain(`${field}:`);
		}

		expect(formVueSource).toMatch(/prop: "initItem"[\s\S]*required: true/);
		expect(formVueSource).toMatch(/props\.mode === "info"/);
	});

	test("dialog close, cancel and submit must unwrap InitializeCellFormData through the shared helper", () => {
		const source = readFileSync(pageSourcePath, "utf8");
		const helperBlock = extractFunctionBlock(source, "getCurrentInitializeCellFormData");
		...
		expect(helperBlock).toMatch(/InitializeCellFormData\s*\|\s*undefined/);
		expect(helperBlock).toContain("initializeCellFormInstance.value?.formComputed");
		...
		expect(helperBlock).toMatch(/cloneDeep\(\s*payload\s*\)\s*as\s*InitializeCellFormData/);

		expect(beforeCloseBlock).toContain("const formData = getCurrentInitializeCellFormData()");
		expect(beforeCloseBlock).toContain("formComputed: formData");
		expect(beforeCloseBlock).toContain("await useDoBeforeClose");
		expect(beforeCloseBlock).not.toContain("initializeCellFormInstance.value?.formComputed");

		expect(cancelButtonBlock).toContain("const formData = getCurrentInitializeCellFormData()");
		expect(cancelButtonBlock).toContain("formComputed: formData");
		expect(cancelButtonBlock).toContain("await useDoBeforeClose");
		expect(cancelButtonBlock).not.toContain("initializeCellFormInstance.value?.formComputed");

		expect(submitButtonBlock).toContain("const formData = getCurrentInitializeCellFormData()");
		expect(submitButtonBlock).toContain("await createInitializeCell(payload)");
		expect(submitButtonBlock).toContain("...formData");
		expect(submitButtonBlock).not.toContain("initializeCellFormInstance.value?.formComputed");
