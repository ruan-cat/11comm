...
/**
 * 根据路由名称获取路由对象
 * @description 后期添加的函数 不是原框架的函数
 */
export function getRouteByName(name: RouteRecordName) {
	if (!name) return null;
...
/**
 * 根据路由名称获取路由完整信息（包含meta等）
 * @description 后期添加的函数 不是原框架的函数
 */
export function getRouteInfoByName(name: RouteRecordName) {
...
/**
 * 404错误报错页面 路由匹配语法
 * @see https://pure-admin.cn/pages/routerMenu/#禁止修改或删除的路由
 */
export const errorRouteGlob = `./modules/**/error.ts`;

/**
 * 首页 路由匹配语法
 * @see https://pure-admin.cn/pages/routerMenu/#禁止修改或删除的路由
 */
export const homeRouteGlob = `./modules/**/home.ts`;

/** `详情页`标识字段 用于标记那些路由属于`详情页` */
export const detailPageFlag = "-detail-page" as const;

/**
 * 当前路由是否是`详情页`路由？
 * @description 根据路径中是否包含`-detail-page`标识来判断
 */
export function isRoutePathDetailPage(path: string) {
