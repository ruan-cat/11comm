	...
	resultOptions.value = flatMenusData.filter((menu) =>
		keyword.value
			? transformI18n(menu.meta?.title).toLocaleLowerCase().includes(keyword.value.toLocaleLowerCase().trim()) ||
				(locale.value === "zh" &&
					!isAllEmpty(
						match(transformI18n(menu.meta?.title).toLocaleLowerCase(), keyword.value.toLocaleLowerCase().trim()),
					))
			: false,
	);
...
<template>
	...
	>
		<el-input
			...
			size="large"
			clearable
			:placeholder="t('common.search.purePlaceholder')"
			@input="handleSearch"
		>
			<template #prefix>
		...
		<div class="search-content">
			<el-scrollbar ref="scrollbarRef" max-height="calc(90vh - 140px)">
				<el-empty v-if="showEmpty" :description="t('common.search.pureEmpty')" />
				<SearchHistory
