...
## 018 <!-- TODO: 持续运行 直到故障被处理 --> 运行后台项目的类型检查命令，整体性的解决项目的类型报错问题

1. 运行后台项目的类型检查命令，并修复类型报错。
...
6. **不要**通过新增 `test-data.ts` 假数据文件的方式，来处理类型故障。
7. **不要**考虑旧中文字段的兼容，不要为了兼容中文字段而去增加中文字段，不要用中文变量来解决类型故障。有问题的是中文变量名，和中文类型名。
...
是接口的筛选逻辑导致的 bug，这个很诡异。不知道为什么，这导致了严重的读取客户端模块的故障。
