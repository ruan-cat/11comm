...
| 文档                                        | 用途                                                  | 阅读时间 |
| :------------------------------------------ | :---------------------------------------------------- | :------: |
| [000-CORE-RULES.md](./000-CORE-RULES.md) 🔴 | **核心规范一页纸** - 最重要的 5 个必须做和 3 个禁止做 |  3 分钟  |
| [001-CHECKLIST.md](./001-CHECKLIST.md) 🔴   | **可执行检查清单** - 迁移单个页面的完整步骤           |  5 分钟  |
| 本文档 (index.md)                           | 完整迁移指南 - 详细的 10 步骤流程                     | 30 分钟  |

**执行建议**：

1. 先快速阅读 `000-CORE-RULES.md` 了解核心要点
2. 按照 `001-CHECKLIST.md` 逐项执行
3. 遇到问题时查阅本文档详细说明
...
1. 阅读充分的行为规范文件：
   - 首先，需要你先阅读 `apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\000-CORE-RULES.md` 文件，这份 `CORE-RULES` 将指导你完成全部的代码改写任务。
   - 其次，需要你先阅读 `.claude\agents\fix-type-error.md` ，确保你在后续的任务内，满足 `fix-type-error` 提及到的规范，避免出现错误的类型使用。
   - 接着，需要你阅读 `apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\001-CHECKLIST.md` `CHECKLIST` 检查清单，确保你在完成任务修改后，都能够严格按照 `CHECKLIST` 清单文件的要求来自检。
   - 最后，全面阅读： `openspec\changes\migrate-static-data-to-nitro-query\specs` 目录内全部的规范文件。这里面包含了具体的规范细则。
2. 及时更新任务清单：
   - 在执行任务时，请及时的更新 `openspec\changes\migrate-static-data-to-nitro-query\tasks.md` 任务进度文件。你每完成一个小任务，就立刻去 `tasks.md` 内更新任务。不要拖延。
3. 禁止编写脚本完成批处理任务：
   > **不允许**你编写任何 Python、typescript、javascript，或 bash 脚本，完成大批量代码删改之类的任务。你应该阅读文件来完成更改，而不是使用不稳定的，容易带来语法错误的，删改不干净不合理的批处理脚本，来完成任务。
4. 主动开启多个后台运行的子代理并行完成任务：
   - 请你主动的开启多个独立并行的修改子代理，加快修改任务。
   - 你新建的子代理**必须**是**后台运行**的子代理。
   - 主代理监控子代理的行为时，如果出现反馈结果为 400，且上下文超限的情况，就认定为子代理执行失败了。请你对子代理的任务做更加精细化的，细粒度的拆分，确保失败的子代理能够处理较少的文件，确保子代理的执行成功率。
5. 子代理工作量划分事项：
   - 不要过于直接的根据业务路径，按照二级路由的业务路径，拆分出过于巨大的模块给子代理完成，这是不对的。这很容易导致子代理无法独立完成全部任务。
   - 你应该给一个子代理划分出 3~5 个业务路径，让子代理完成这 3~5 个业务路径内的代码更改任务。这样才能保证子代理完成的内容足够少，确保子代理完成最终任务的成功率。
6. 大部分文件需要仔细复查：

   > 有绝大多数的文件，都不同程度按照要求来做，但是这些代码做的都缺斤少两，缺少很多代码写法。在你批量的，按照要求修改代码时，务必要**仔细**的阅读每一份文件，仔细的阅读对照规范文档的修改要求。你应该新建一个专门的子代理，将现有的代码做非常严格的检查。按照以下步骤作为检查清单，结合规范文档，逐个检查，排查，补全代码写法：
   - 在类型项目内，是否缺少需要迁移的业务类型？
   - 在类型项目内，是否错误迁移了形如 `xxxFormProps` 的表格类型？
   - 在后台项目内，是否新建了正确的 api hooks？是否补全了必填项参数？
   - 在列表页内，是否正确使用了 api hooks？是否按照严格的要求改写代码？
   - 在 form.ts 内，是否更换中文类型？是否有 `xxxFormProps` 的表格类型？
   - 在 form.vue 内，是否有正确使用下拉选项数组？
...
### 019 不要将非业务类型迁移到类型项目内，特别是表单弹框组件类型
...
1. 阅读以下严格的规范和要求文件：
   - apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\019.md
   - apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\016.md
   - apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\017.md
