...
1. 运行`后台项目`的类型检查命令，并修复类型报错。
2. 当你遇到那种因为缺失类型和变量的错误时，请你根据文件的路径，去智能的推断其应该在`类型项目`内的变量名和类型名。
   - 各种缺失的类型和变量，应该首先去 `@01s-11comm/type` 内寻找。请你按照文件路径所体现的业务路径，在 `@01s-11comm/type` 内去索引正确的变量。
   - 需要找的变量通常是类型，和下拉选择数组。
3. 请你严格按照 `openspec\changes\migrate-static-data-to-nitro-query` 目录内的全部文件，来更改代码。
4. 很多类型文件，为了完成类型修复，而被错误的改写成中文。这是不对的，不符合 `migrate-static-data-to-nitro-query` 任务的要求的。在你修复类型报错的时候，请你在将中文命名的字段换成英文字段的前提下，对应的，及时的去修改 vue 组件内使用的中文命名。
5. 请主动的使用 `fix-type-error` 子代理，完成类型修复任务。
   - 先针对性的确保类型项目不出现故障。
   - 再去处理 admin 后台项目的类型故障。
6. 禁止编写脚本完成批处理任务： **不允许**你编写任何 Python、typescript、javascript，或 bash 脚本，完成大批量代码删改之类的任务。你应该阅读文件来完成更改，而不是使用不稳定的，容易带来语法错误的，删改不干净不合理的批处理脚本，来完成任务
7. **不要**通过新增 `test-data.ts` 假数据文件的方式，来处理类型故障。
8. **不要**考虑旧中文字段的兼容，不要为了兼容中文字段而去增加中文字段，不要用中文变量来解决类型故障。有问题的是中文变量名，和中文类型名。
