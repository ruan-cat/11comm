...
1. **全局核心规范**：`apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\000-CORE-RULES.md`
2. **类型安全规范**：`.claude\agents\fix-type-error.md`（严禁出现任何隐式 Any 或类型断言错误）
3. **验收检查清单**：`apps\admin\src\docs\prompts\各种杂项\2025-12-12-migrate-static-data-to-nitro-query\001-CHECKLIST.md`
4. **具体业务规格**：`openspec\changes\migrate-static-data-to-nitro-query\specs` 目录下的所有文件。
...
- **状态同步（解决冲突）**：**仅由主代理**统一负责更新 `openspec\changes\migrate-static-data-to-nitro-query\tasks.md`。子代理完成任务后向主代理汇报，主代理核实后立即更新，确保进度文件不发生写入冲突。
