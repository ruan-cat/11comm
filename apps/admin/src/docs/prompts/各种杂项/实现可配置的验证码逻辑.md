...
1. `项目配置文件`： `apps\admin\public\platform-config.json`
2. `PlatformConfigs`： `项目配置文件`对应的类型。类型文件在 `apps\admin\types\global.d.ts` 内
3. `登录页`： `apps\admin\src\views\login\index.vue`
4. `可配置验证码文档目录`： `apps\admin\src\docs\feat\configurable-verify-code`
