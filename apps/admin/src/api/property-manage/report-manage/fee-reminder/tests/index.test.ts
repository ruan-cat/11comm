...
type FeeReminderApiModule = typeof import("../index");

async function importFeeReminderApi(env: Record<string, string>): Promise<FeeReminderApiModule> {
	vi.unstubAllEnvs();
	vi.resetModules();
...
describe("fee-reminder admin api", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.unstubAllEnvs();
	});
	...
	test("keeps list query on legacy relative api path when shadow is disabled", async () => {
		const { useFeeReminderListQuery } = await importFeeReminderApi({
