...
type NoChargeHouseApiModule = typeof import("../index");

async function importNoChargeHouseApi(env: Record<string, string>): Promise<NoChargeHouseApiModule> {
	vi.unstubAllEnvs();
	vi.resetModules();
...
describe("no-charge-house admin api", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.unstubAllEnvs();
	});
	...
	test("keeps list query on legacy relative api path when shadow is disabled", async () => {
		const { useNoChargeHouseListQuery } = await importNoChargeHouseApi({
		...
		useNoChargeHouseListQuery({ ownerName: "owner" });
