...
type StatementExpensesApiModule = typeof import("../index");

async function importStatementExpensesApi(env: Record<string, string>): Promise<StatementExpensesApiModule> {
	vi.unstubAllEnvs();
	vi.resetModules();
...
describe("statement-expenses admin api", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.unstubAllEnvs();
	});
	...
	test("keeps list query on legacy relative api path when shadow is disabled", async () => {
		const { useStatementExpensesListQuery } = await importStatementExpensesApi({
		...
		useStatementExpensesListQuery({ expenseType: "statement" });

		expect(mocks.useListQuery).toHaveBeenCalledWith({
			queryKeyPrefix: "statementExpenses",
			apiUrl: API_URL,
			initialParams: { expenseType: "statement" },
		});
	});
