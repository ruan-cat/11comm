...
type OwnerPaymentDetailsApiModule = typeof import("../index");

async function importOwnerPaymentDetailsApi(env: Record<string, string>): Promise<OwnerPaymentDetailsApiModule> {
	vi.unstubAllEnvs();
	vi.resetModules();
...
describe("owner-payment-details admin api", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.unstubAllEnvs();
	});
	...
	test("keeps list query on legacy relative api path when shadow is disabled", async () => {
		const { useOwnerPaymentDetailsListQuery } = await importOwnerPaymentDetailsApi({
