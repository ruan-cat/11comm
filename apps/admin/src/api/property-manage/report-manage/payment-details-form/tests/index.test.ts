...
const API_URL = "/api/property-manage/report-manage/payment-details-form/list";

type PaymentDetailsFormApiModule = typeof import("../index");

async function importPaymentDetailsFormApi(env: Record<string, string>): Promise<PaymentDetailsFormApiModule> {
	vi.unstubAllEnvs();
	vi.resetModules();
...
describe("payment-details-form admin api", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.unstubAllEnvs();
	});
	...
	test("keeps list query on legacy relative api path when shadow is disabled", async () => {
		const { usePaymentDetailsFormListQuery } = await importPaymentDetailsFormApi({
		...
		usePaymentDetailsFormListQuery({ name: "cash" });

		expect(mocks.useListQuery).toHaveBeenCalledWith({
			queryKeyPrefix: "paymentDetailsForm",
			apiUrl: API_URL,
			initialParams: { name: "cash" },
		});
	});
