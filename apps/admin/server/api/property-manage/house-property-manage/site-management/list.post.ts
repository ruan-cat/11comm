/**
 * @file 场地管理列表接口
 * @description SiteManagement list API
 * POST /api/property-manage/house-property-manage/site-management/list
 */

import { defineHandler, readBody } from "nitro/h3";
import type { JsonVO, PageDTO, SiteManagementListItem, SiteManagementQueryParams } from "@01s-11comm/type";
...
export default defineHandler(async (event): Promise<JsonVO<PageDTO<SiteManagementListItem>>> => {
	...
	const defaultParams: SiteManagementQueryParams = {
	...
	// 2. 数据筛选 - 使用通用筛选工具函数
	const filteredData = filterDataByQuery(mockSiteManagementData, filters);
