...
export default defineHandler(async (event): Promise<JsonVO<PageDTO<PaymentDetailsFormListItem>>> => {
	try {
		...
		const list: PaymentDetailsFormListItem[] = data.map((item) => ({
			...
			orderNumber: item.transactionNo || item.id || "",
			community: "",
			roomNumberOwner: [item.houseNumber, item.ownerName].filter(Boolean).join(" / "),
			feeType: item.expenseItem || "",
			feeItem: item.expenseItem || "",
			feeStatus: item.paymentAmount ? "已支付" : "未支付",
			paymentMethod: item.paymentMethod || "",
			paymentTime: formatDateTime(item.paymentTime),
			cashier: item.collector || "",
			payableAmount: String(item.paymentAmount || "0"),
			receivableAmount: String(item.paymentAmount || "0"),
			actualAmount: String(item.paymentAmount || "0"),
			accountDeduction: "0",
			discountAmount: "0",
			giftAmount: "0",
			lateFee: "0",
