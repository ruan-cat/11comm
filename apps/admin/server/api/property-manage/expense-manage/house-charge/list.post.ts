...
import { exHouseCharges } from "@01s-11comm/type";
...
export default defineHandler(async (event): Promise<JsonVO<PageDTO<any>>> => {
	try {
		...
		const rawQuery = {
			...
			billingPeriod: body.billingPeriod === "" ? undefined : body.billingPeriod,
		...
		if (query.expenseItem) {
			conditions.push(like(exHouseCharges.expenseItem, `%${query.expenseItem}%`));
		}

		if (query.billingPeriod) {
			conditions.push(like(exHouseCharges.billingPeriod, `%${query.billingPeriod}%`));
		}

		if (query.status) {
			conditions.push(eq(exHouseCharges.status, query.status as any));
		}
		...
		const data = await db
			.select({
				id: exHouseCharges.id,
				houseId: exHouseCharges.houseId,
				expenseItem: exHouseCharges.expenseItem,
				receivableAmount: exHouseCharges.receivableAmount,
				receivedAmount: exHouseCharges.receivedAmount,
				billingPeriod: exHouseCharges.billingPeriod,
				status: exHouseCharges.status,
		...
		console.error("[House Charge List] Error:", error);
