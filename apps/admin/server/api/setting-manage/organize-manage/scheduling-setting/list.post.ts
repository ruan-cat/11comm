...
export default defineHandler(async (event): Promise<JsonVO<PageDTO<SchedulingSetting>>> => {
	// 1. 读取请求参数
	const body = await readBody<SchedulingSettingListQuery>(event);
	const defaultParams: SchedulingSettingListQuery = {
	...
	// 2. 数据筛选 - 使用通用筛选工具函数
	const filteredData = filterDataByQuery(mockSchedulingSettingData, filters);
	...
	/** 返回标准格式 */
	const response: JsonVO<PageDTO<SchedulingSetting>> = {
		success: true,
		code: 200,
		message: "查询成功",
		data: {
