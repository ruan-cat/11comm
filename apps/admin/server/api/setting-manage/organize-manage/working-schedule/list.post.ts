...
export default defineHandler(async (event): Promise<JsonVO<PageDTO<WorkingSchedule>>> => {
	// 1. 读取请求参数
	const body = await readBody<WorkingScheduleListQuery>(event);
	const defaultParams: WorkingScheduleListQuery = {
	...
	// 2. 数据筛选 - 使用通用筛选工具函数
	const filteredData = filterDataByQuery(mockWorkingScheduleData, filters);
	...
	/** 返回标准格式 */
	const response: JsonVO<PageDTO<WorkingSchedule>> = {
		success: true,
		code: 200,
		message: "查询成功",
		data: {
