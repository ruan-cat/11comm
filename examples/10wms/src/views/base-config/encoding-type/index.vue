...
import {
	addCodeBaseConfig,
	deleteCodeBaseConfig,
	exportCodeBaseConfig,
	listCodeBaseConfig,
	updateCodeBaseConfig,
} from "@/apis/encoding-type-gaogao/index";
...
const componentsTableProps = ref<SimpleDataTableProps<TableData>>({
	...
	data: [],
	columns: [
		{ prop: "snroTypeCode", label: "编码类型代码" },
		{ prop: "snroTypeName", label: "编码类型名称" },
...
const dialogPromiseProps = ref<DialogPromiseProps<TableData>>({
	dialogProps: {
		// title: "编码类型录入",
		title: isViewMode.value ? "编码类型查看" : "编码类型录入",
	},
...
function handleEdit() {
	...
	form.value = {
		...
		snroTypeName: selectedRows.value[0].snroTypeName ?? "",
	}; // 填充表单数据
	isViewMode.value = false; // 编辑模式
	dialogType.value = 1;
	openDialog(); // 打开弹窗
		...
		<ComponentsDialogPromise :="dialogPromiseProps" ref="dialog">
			<template #default>
				<ElForm :model="form" label-position="right" label-width="150px">
					<h2>表单信息管理</h2>
					<ElFormItem prop="snroTypeCode" label="编码类型代码：">
						<ElInput v-model="form.snroTypeCode" :readonly="isViewMode" />
					</ElFormItem>
					<ElFormItem prop="snroTypeName" label="编码类型名称：">
						<ElInput v-model="form.snroTypeName" :readonly="isViewMode" />
					</ElFormItem>
				</ElForm>
			</template>
