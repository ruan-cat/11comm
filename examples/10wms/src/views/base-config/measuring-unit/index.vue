...
const componentsTableProps = ref<SimpleDataTableProps<TableData>>({
	...
	data: [],
	columns: [
		{ prop: "unitCode", label: "单位代码" },
		{ prop: "unitZhName", label: "单位名称" },
		{ prop: "unitEnName", label: "英文名称" },
...
const dialogPromiseProps = ref<DialogPromiseProps<TableData>>({
	dialogProps: {
		// title: "计量单位录入",
		title: isViewMode.value ? "查看计量单位" : "编辑计量单位",
	},
...
function handleEdit() {
	...
	form.value = {
		...
		unitCode: selectedRows.value[0].unitCode ?? "",
		unitZhName: selectedRows.value[0].unitZhName ?? "",
		unitEnName: selectedRows.value[0].unitEnName ?? "",
	}; // 填充表单数据
	isViewMode.value = false; // 编辑模式
	dialogType.value = 1;
	openDialog(); // 打开弹窗
		...
		<ComponentsDialogPromise :="dialogPromiseProps" ref="dialog">
			<template #default>
				<ElForm :model="form" label-position="right" label-width="150px">
					<h2>表单信息管理</h2>
					<ElFormItem prop="unitCode" label="单位代码：">
						<ElInput v-model="form.unitCode" :readonly="isViewMode" />
					</ElFormItem>
					<ElFormItem prop="unitZhName" label="单位名称：">
						<ElInput v-model="form.unitZhName" :readonly="isViewMode" />
					</ElFormItem>
					<ElFormItem prop="unitEnName" label="英文名称：">
						<ElInput v-model="form.unitEnName" :readonly="isViewMode" />
					</ElFormItem>
				</ElForm>
			</template>
