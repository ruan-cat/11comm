...
- [x] **2.3 消费者验证 (试点测试)**
  ...
  - **状态**: ✅ typecheck 通过，枚举已扩展到 common/enums.ts
...
- [x] **3.1.1 社区管理 (Community Manage)** (`apps/type/src/business/property-manage/community-manage/schema.ts`)
- [x] **3.1.2 房产管理 (House Property)** (`apps/type/src/business/property-manage/house-property-manage/schema.ts`)
- [x] **3.1.3 停车管理 (Parking Manage)** (`apps/type/src/business/property-manage/parking-manage/schema.ts`)

### Group 3.2: 物业管理 B (Sub-Agent D)

- [x] **3.2.1 合同管理 (Contract Manage)** (`apps/type/src/business/property-manage/contract-manage/schema.ts`)
- [x] **3.2.2 费用管理 (Expense Manage)** (`apps/type/src/business/property-manage/expense-manage/schema.ts`)
  - _注意_: 该模块可能包含多个表。将它们放在同一个 `schema.ts` 中，或者如果有必要则拆分，但必须统一导出。
- [x] **3.2.3 报修管理 (Repairs Manage)** (`apps/type/src/business/property-manage/repairs-manage/schema.ts`)

### Group 3.4: 补充模块 (Additional Modules) (Sub-Agent H)

- [x] **3.4.1 巡检管理 (Patrol Manage)** (`apps/type/src/business/property-manage/patrol-manage/schema.ts`)
- [x] **3.4.2 报表管理 (Report Manage)** (`apps/type/src/business/property-manage/report-manage/schema.ts`)

- [x] **3.5 全面审计 (Comprehensive Audit)**
  - **动作**: 对比原 `db/schemas` 12 个文件与 `apps/type` 新生成的 14 个 `schema.ts` 文件。
  - **结果**: 确认 100% 覆盖 Drizzle 表定义、枚举定义及辅助函数。确认导出链条完整。
