...
**CRITICAL**: 在实施下拉选项迁移时，必须严格按照以下顺序执行，不允许跳步。

**执行顺序:**

1. **Step 1**: 识别 form.ts 文件中的下拉选项数组
2. **Step 2**: 判断是否为公共选项
3. **Step 3**: 迁移到 business-options.ts 或业务类型文件
4. **Step 4**: 更新 form.ts 文件导入
5. **Step 5**: 类型检查验证

**步骤依赖关系:**

- Step 1 是识别阶段，找出所有的下拉选项定义
- Step 2 是决策阶段，判断选项是否应该作为公共选项
- Step 3 是迁移阶段，将选项迁移到合适的位置
