...
### 需求：Nitro 数据库原生交互标准 (Nitro Native DB Interaction Standard)

Nitro 服务器的所有 API 事件处理程序（Event Handlers）必须废弃静态 Mock 数据，转而使用 Drizzle ORM 直接与 Neon 数据库进行交互，以实现数据的真实持久化和状态管理。

#### 场景：写入操作持久化 (Write Persistence)
...
#### 场景：读取操作实时性 (Read Real-time Consistency)
...
### 需求：全栈 Schema 驱动验证 (Full-Stack Schema-Driven Validation)

API 层必须建立“零信任”机制。所有进入系统的外部输入（Body, Query Params, Route Params）必须经过 `apps/type` 中定义的 Zod Schema 的严格校验。

#### 场景：请求体严格校验 (Strict Body Validation)
...
#### 场景：查询参数与路由参数校验 (Query & Param Validation)
...
### 需求：同构类型架构 (Isomorphic Type Architecture)

`apps/type` 必须构建为全栈共享的运行时库，确保前后端使用完全一致的数据定义。

#### 场景：单一事实来源引用 (Single Source of Truth Reference)
...
### 需求：自动化错误语义映射 (Automated Error Semantic Mapping)

后端必须具备将底层数据库技术错误自动转化为上层业务语义错误的能力。

#### 场景：唯一性冲突处理 (Uniqueness Conflict Handling)
...
#### 场景：引用完整性处理 (Referential Integrity Handling)
