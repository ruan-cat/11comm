...
## 1. 基础设施与环境准备 (Infrastructure)

- [ ] 1.1 **升级 `apps/type` 运行时依赖**
  - 进入 `apps/type` 目录。
...
## 3. Schema 迁移与 API 重写 - 核心模块 (Core Modules)

### 3.1 字典管理 (dev-team/config-manage/dictionary)

- [ ] 3.1.1 **Schema define**: 创建 `apps/type/src/business/dev-team/config-manage/dictionary/schema.ts`
