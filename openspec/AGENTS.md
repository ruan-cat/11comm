...
- [ ] **确认类型库**：检查新增功能是否需要添加类型定义到 `@01s-11comm/type` 包
...
项目新增了 `@01s-11comm/type` 包，这是一个专门的业务类型库，用于集中管理所有共享的 TypeScript 类型定义。

**位置**：`apps/type/src/index.ts`

**使用原则**：

1. 所有业务相关的类型定义都应该添加到类型库中
2. 确保类型定义的一致性和准确性
3. 避免在各个包中重复定义相同的类型
...
1. 在开发新功能时，首先检查是否需要新增或修改类型定义
2. 将类型定义添加到 `@01s-11comm/type` 包中
3. 在相关包中通过 `workspace:^` 引用类型库
4. 运行类型检查确保类型安全
...
常见陷阱：使用 MODIFIED 添加新关注点而不包含之前的文本。这会在归档时丢失详细信息。如果您没有明确更改现有需求，而是在 ADDED 下添加新需求。
...
- 使用 kebab-case 格式，简短且描述性：`add-two-factor-auth`
