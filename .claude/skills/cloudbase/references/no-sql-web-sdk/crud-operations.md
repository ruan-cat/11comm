...
```javascript
// Add with custom ID
const result = await db.collection("todos").doc("custom-todo-id").set({
...
const results = await Promise.all(todos.map((todo) => db.collection("todos").add(todo)));
...
await db.collection("todos").doc("todo-id-123").update({
	title: "Updated Title",
...
await db.collection("profiles").doc("profile-123").update({
...
await db.collection("todos").doc("todo-123").update({ title: "New Title" });
```
...
await db.collection("todos").doc("todo-123").set({ title: "New Title", completed: false });
```
...
class TodoManager {
	...
	async getTodos(filter = {}) {
		const result = await this.collection.where(filter).orderBy("createdAt", "desc").get();
		return result.data;
	}
	...
	async deleteCompleted() {
		const result = await this.collection.where({ completed: true }).remove();
		return result.deleted;
	}
}
...
// Create
const todoId = await todoManager.createTodo("Learn CloudBase", "Study the database API", "high");
