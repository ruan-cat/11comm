...
| property-manage/community-manage      |         7 | exact 7                    | `available-in-apps-api-not-caller-verified`；2026-05-19 已补 runtime manifest/contract/HTTP gate，仍缺生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互和旧服务退役证据 |
...
| property-manage/house-property-manage |        10 | exact 10                   | `available-in-apps-api-not-caller-verified`；2026-05-19 已补 runtime manifest/contract/HTTP gate，仍缺生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互和旧服务退役证据 |
...
| `propertyManage.repairsManage.repairsTodo`、`propertyManage.repairsManage.issues`                                                                                 | `POST /api/property-manage/repairs-manage/repairs-todo/list`、`POST /api/property-manage/repairs-manage/issues/list`                                                                           | `GET/POST /app/ownerRepair.listOwnerRepairs`                      | `apps/api` repair legacy adapter | legacy `{ code, msg, data: { ownerRepairs, total } }`    | repair runtime：DB read repository / memory fallback            | 已进 app shadow allowlist；未开启 shadow 时仍可回旧 app runtime           | `apps/api/tests/legacy/repair-legacy-endpoints.test.ts`、manifest、app fallback allowlist test、App H5 页面 Network                                       | 2026-05-18 本地 App H5 页面上下文 Network 已记录：命中 apps/api 3102，返回 200、`x-api-phase=phase3-infra`、`code=0`、`ownerRepairs.length=1`；证据 `.tmp/phase7-dev-browser/2026-05-18-app-repair-readonly-h5-network-verification.md`           | `candidate-after-evidence`；仍缺生产 DB_READY、真实库样本、shadow-off/fallback 和独立复核 |
| `propertyManage.repairsManage.repairsTodo`                                                                                                                        | 当前 `apps/api` 无 repair detail admin route                                                                                                                                                   | `GET/POST /app/ownerRepair.queryOwnerRepair`                      | `apps/api` repair legacy adapter | legacy `{ code, msg, data: { ownerRepair } }`            | repair runtime：DB read repository / memory fallback            | 已进 app shadow allowlist；未开启 shadow 时仍可回旧 app runtime           | `apps/api/tests/legacy/repair-legacy-endpoints.test.ts`、manifest、App H5 页面 Network                                                                    | 2026-05-18 本地 App H5 页面上下文 Network 已记录：使用 list 返回的真实 UUID 命中 apps/api 3102，返回 200、`x-api-phase=phase3-infra`、`code=0`；证据同上。文档示例 `REP_001` 在 DB-backed 路径下不是有效 UUID，不能作为详情证据 ID                | `candidate-after-evidence`；仍缺生产 DB_READY、真实库样本、shadow-off/fallback 和独立复核 |
...
| `propertyManage.repairsManage.repairsSetting`                                                                                                                     | `POST /api/property-manage/repairs-manage/repairs-setting/list`                                                                                                                                | `GET/POST /app/repairSetting.listRepairSettings`                  | `apps/api` repair legacy adapter | legacy `{ code, msg, data: [] }`                         | repair runtime：DB read repository / memory fallback            | 已进 app shadow allowlist；未开启 shadow 时仍可回旧 app runtime           | `apps/api/tests/legacy/repair-legacy-endpoints.test.ts`、manifest、app fallback allowlist test                                                            | 未记录单独 App H5 页面级 evidence；仍需补 `/app/repairSetting.listRepairSettings` 页面上下文 Network                                                                                                                                              | `candidate-after-evidence`；仍缺生产 DB_READY、真实库样本、shadow-off/fallback 和独立复核 |
| `propertyManage.repairsManage.repairsSetting`                                                                                                                     | 当前 `apps/api` 无独立 status dict admin route                                                                                                                                                 | `GET/POST /app/dict.queryRepairStates`                            | `apps/api` repair legacy adapter | legacy `{ code, msg, data: [{ statusCd, name }] }`       | repair runtime：DB read repository / memory fallback            | 已进 app shadow allowlist；未开启 shadow 时仍可回旧 app runtime           | `apps/api/tests/legacy/repair-legacy-endpoints.test.ts`、`apps/api/tests/infra/endpoint-manifest.test.ts`、App H5 页面 Network                            | 2026-05-18 本地 App H5 repair list 搜索栏页面上下文 Network 已记录：命中 apps/api 3102，返回 200、`x-api-phase=phase3-infra`、`code=0`、`data.length=6`；证据 `.tmp/phase7-dev-browser/2026-05-18-app-repair-readonly-h5-network-verification.md` | `candidate-after-evidence`；仍缺生产 DB_READY、真实库样本、shadow-off/fallback 和独立复核 |

特别说明：`/app/ownerRepair.saveOwnerRepair` 与 repair 只读端点已存在于 `apps/api` manifest 与 legacy endpoint 测试中；`/app/ownerRepair.listOwnerRepairs`、`/app/ownerRepair.queryOwnerRepair`、`/app/repairSetting.listRepairSettings`、`/app/dict.queryRepairStates` 当前已纳入 app shadow allowlist，其中前三者里的 list/detail/dict 三个只读页面请求已在 2026-05-18 补本地 App H5 页面上下文 Network 证据。该证据只能升级对应本地 `browserEvidence`，不能替代生产 `DB_READY`、真实库样本、shadow-off/fallback、写入口回滚或旧服务退役评审；`/app/repairSetting.listRepairSettings` 仍需单独页面证据，`/app/ownerRepair.saveOwnerRepair` 仍保持高风险写入口 guard。
...
- 最新事实源按主控验证与当前扫描为准：admin 旧路径 exact coverage 为 155/155，`apps/api/server/routes/api/**/*.ts` 为 160 个 server route；`property-manage/report-manage/expense-summary-table/list` 已独立实现并接 resolver，admin 前端 resolver 已全完成。2026-05-18 已补齐 `repairs-manage` 7 个 admin list old path 的 runtime manifest/contract/HTTP gate 证据：`runtime-endpoints.ts` 新增 `repairs-have-done/list`、`return-visit/list`、`phone-report-repairs/list`、`mandatory-return-issue/list` 四个 admin canonical manifest 条目；目标 Vitest 3 files / 12 tests passed，fresh `apps/api` 3112 gated HTTP 1 file / 6 tests passed，api typecheck 与 `git diff --check` 通过。该证据不得升级为生产 `DB_READY`、真实页面 CRUD 交互、shadow-off/fallback 或旧服务退役。
- 2026-05-19 已补齐 `operation-team` 13 个 admin list old path 的 runtime manifest/contract/HTTP gate 护栏：`runtime-endpoints.ts` 新增 13 个 `admin-canonical` + `JsonVO` manifest 条目，`ownerModule=operation`，`cutoverStatus=available-in-apps-api-not-caller-verified`；infra manifest/contract tests 与 fetch 型 gated HTTP POST test 显式覆盖。验证：目标 Vitest 3 files / 12 tests passed；fresh `apps/api` 3114 gated HTTP 1 file / 7 tests passed；api typecheck 与 diff check 通过。该证据只代表本地 apps/api 可达与 JsonVO contract，仍不得写成生产 `DB_READY`、真实库样本、shadow-off/fallback 或旧服务可退役。
- 2026-05-19 继续补齐 `patrol-manage` 6 个与 `parking-manage` 4 个 admin list old path 的 runtime manifest/contract/HTTP gate 护栏：`runtime-endpoints.ts` 新增 10 个 `admin-canonical` + `JsonVO` manifest 条目，`ownerModule=patrol|parking`，`cutoverStatus=available-in-apps-api-not-caller-verified`；infra manifest/contract tests 与 fetch 型 gated HTTP POST test 显式覆盖。验证：目标 Vitest 3 files / 12 tests passed；fresh `apps/api` 3115 gated HTTP 1 file / 8 tests passed；api typecheck 与 cached diff check 通过。该证据只代表本地 apps/api 可达与 JsonVO contract，仍不得写成生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互或旧服务可退役。
- 2026-05-19 继续补齐 `property-manage/expense-manage` 14 个 admin list old path 的 runtime manifest/contract/HTTP gate 护栏：`runtime-endpoints.ts` 新增 14 个 `admin-canonical` + `JsonVO` manifest 条目，`ownerModule=fee`，`cutoverStatus=available-in-apps-api-not-caller-verified`；infra manifest/contract tests 与 fetch 型 gated HTTP POST test 显式覆盖。验证：目标 Vitest 3 files / 12 tests passed；fresh `apps/api` 3116 gated HTTP 1 file / 9 tests passed；api typecheck 与 cached diff check 通过。该证据只代表本地 apps/api 可达与 JsonVO contract，仍不得写成生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互或旧服务可退役。
- 2026-05-19 继续补齐 `property-manage/report-manage` 12 个 admin list old path 的 runtime manifest/contract/HTTP gate 护栏：`runtime-endpoints.ts` 新增 12 个 `admin-canonical` + `JsonVO` manifest 条目，`ownerModule=fee-report`，`cutoverStatus=available-in-apps-api-not-caller-verified`；infra manifest/contract tests 与 fetch 型 gated HTTP POST test 显式覆盖。验证：目标 Vitest 3 files / 12 tests passed；fresh `apps/api` 3117 gated HTTP 1 file / 10 tests passed；api typecheck 与 cached diff check 通过。该证据只代表本地 apps/api 可达与 JsonVO contract，仍不得写成生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互或旧服务可退役。
- 2026-05-19 继续补齐 `property-manage/community-manage` 7 个 admin list old path 的 runtime manifest/contract/HTTP gate 护栏：`runtime-endpoints.ts` 新增 7 个 `admin-canonical` + `JsonVO` manifest 条目，`ownerModule=community`，`cutoverStatus=available-in-apps-api-not-caller-verified`；infra manifest/contract tests 与 fetch 型 gated HTTP POST test 显式覆盖。验证：目标 Vitest 3 files / 12 tests passed；fresh `apps/api` 3118 gated HTTP 1 file / 11 tests passed；api typecheck 与 diff check 通过；复核子代理确认无缺漏、未改业务 handler/旧服务目录。该证据只代表本地 apps/api 可达与 JsonVO contract，仍不得写成生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互或旧服务可退役。
- 2026-05-19 继续补齐 `property-manage/house-property-manage` 10 个 admin list old path 的 runtime manifest/contract/HTTP gate 护栏：`runtime-endpoints.ts` 新增 10 个 `admin-canonical` + `JsonVO` manifest 条目，`ownerModule=house-property`，`cutoverStatus=available-in-apps-api-not-caller-verified`；infra manifest/contract tests 与 fetch 型 gated HTTP POST test 显式覆盖，并补齐 `phase7-house-property-manage-admin-list` 断言。验证：目标 Vitest 3 files / 12 tests passed；fresh `apps/api` 3119 gated HTTP 1 file / 12 tests passed；api typecheck 与 diff check 通过；复核子代理确认 `phase` 断言完整且未破坏其他批次语义。该证据只代表本地 apps/api 可达与 JsonVO contract，仍不得写成生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互或旧服务可退役。
- 2026-05-19 中断接力状态：已按用户要求停止继续扩展并关闭 `contract-manage` 编辑子代理；该子代理未落盘 contract 变更。主控只读扫描确认 `property-manage/contract-manage` 12 个普通 list endpoint 仍缺 runtime manifest/contract/HTTP gate：`archive/list`、`attachment/list`、`clause/list`、`change/list`、`draft-contract/list`、`expire/list`、`first-party/list`、`print/list`、`review/list`、`second-party/list`、`template/list`、`type/list`。下一轮可只补这 12 个 list；`upload/{init,sign-part,complete,status,abort}`、`change/{create,update,delete,detail}`、`draft-contract/{create,update,delete,detail}` 继续排除并单独评审。
...
- App legacy 21 endpoint 证据只保持 Nitro manifest / allowlist / guard / fallback 口径；2026-05-18 已补 App repair 只读三端点 `/app/ownerRepair.listOwnerRepairs`、`/app/dict.queryRepairStates`、`/app/ownerRepair.queryOwnerRepair` 的本地 App H5 页面上下文 Network 证据，但仍不能写成生产 `DB_READY`、shadow-off/fallback、写入口闭环或退役证据。
...
- 下一步是继续补其他未覆盖页面 Network / HTTP gate、CRUD DB/R2、生产 `DB_READY`、App 端后续批次、真实库样本复核、shadow-off/fallback、必要写入回滚和独立复核证据；`report-manage/expense-summary-table/list`、P1 `report-manage` 剩余 7 页、App repair 只读三端点 `/app/ownerRepair.listOwnerRepairs`、`/app/dict.queryRepairStates`、`/app/ownerRepair.queryOwnerRepair`、`repairs-manage` 7 个 admin list old path、`operation-team` 13 个 admin list old path、`patrol-manage`/`parking-manage` 10 个 admin list old path、`expense-manage` 14 个 admin list old path、`report-manage` 12 个 admin list old path、`community-manage` 7 个 admin list old path、以及 `house-property-manage` 10 个 admin list old path 的 manifest/contract/HTTP gate 不再列为本地待补。中断后下一轮建议从 `contract-manage` 12 个普通 list endpoint 继续；upload/R2、写入、删除、detail 继续排除。仍缺生产 `DB_READY`、真实库样本、shadow-off/fallback、真实页面 CRUD/交互和旧服务退役证据；退役评估仍保持 no-go。
