...
| Admin 前端切流     | `report-manage` exact route hooks 已补：`payment-details-form`、上一轮 P1 四端点和本轮 7 个 exact route 均已使用 `resolveAdminApiRequestUrl` 并补 hook tests。2026-05-12 batch list resolver 新增 6 个 admin hook resolver + hook tests：`setting-manage/system-manage/change-password/list`、`setting-manage/system-manage/community-configuration/list`、`setting-manage/system-manage/register-protocol/list`、`dev-team/config-manage/type/list`、`dev-team/config-manage/item/list`、`dev-team/config-manage/center/list`；主控复验 4 files / 18 tests passed。2026-05-16 继续补齐 3 个 hook-level resolver + hook tests：`setting-manage/system-manage/system-config/list`、`setting-manage/system-manage/initialize-cell/list`、`dev-team/config-manage/dictionary/list`；主控复验 3 files / 18 tests passed。2026-05-16 本轮继续补齐 `operation-team/data-manage` 3 个 hook resolver + tests：`community-information/list`、`property-company/list`、`property-management-company/list`；主控复验 1 file / 9 tests passed；补齐 `operation-team/system-manage` Core 3 个 hook resolver + tests：`change-password/list`、`system-config/list`、`register-protocol/list`；主控复验 1 file / 9 tests passed；补齐 `expense-manage` Slice A/B 共 6 个 hook resolver + tests：`cancel-fee/list`、`contracte-charge/list`、`discount-apply/list`、`discount-setting/list`、`discount-type/list`、`expense-summary-table/list`；主控复验 2 files / 18 tests passed。2026-05-16 追加补齐 `operation-team/system-manage` 剩余 2 个、`operation-team/merchant-manage` 2 个、`operation-team/report-configuration` 3 个 hook resolver + tests；operation-team 统一主控复验 5 files / 39 tests passed。2026-05-16 继续补齐 `expense-manage` Slice C/D/E 共 8 个 hook resolver + tests：`meter-reading-type/list`、`overdue-payment-information/list`、`payment-review/list`、`refund-review/list`、`reminder-for-overdue-payments/list`、`reprint-voucher/list`、`vehicle-charge/list`、`water-and-electricity-meter-reading/list`；expense-manage A-E 统一主控复验 5 files / 42 tests passed，admin typecheck 通过，目标硬编码扫描清零。`report-manage/expense-summary-table/list` 因 `apps/api` 不存在对应 report-manage 路径、仅存在 `expense-manage/expense-summary-table/list.post.ts`，仍按路径冲突和硬编码保留，不写成已切流。剩余代表性 resolver 缺口不再包括 operation-team 或 expense-manage，仍包括 `community-manage`、`dev-team/menu-manage` / `cache-manage` 等。 |
...
- [ ] 下一轮继续处理 `expense-summary-table` 的 report-manage / expense-manage 路径冲突和剩余 resolver 缺口（`community-manage`、`dev-team/menu-manage` / `cache-manage` 等）；再做 Neon main `PHASE7_E2E_*` 写入-回读-清理或只读 DB 验收、shadow-off/fallback 页面演练，最后才评估旧服务退役。
...
1. P1 四个 report-manage 端点已完成一轮本地语义验收：`owner-payment-details/list`、`repair-report-form/list`、`repair-reports-summary-table/list`、`statement-expenses/list`。
2. 这 4 个端点已补字段映射、过滤条件、JSONB 解析/聚合语义和 Vitest；但当前状态仍只能写 `old-path-exact-covered (working-tree-staged)`、`db-read-repository-wired-with-gap`、`unknown-needs-triage`、`keep-source`。
3. `report-manage` exact route hooks 已补前端 API hooks 与 `resolveAdminApiRequestUrl`：`payment-details-form`、P1 四端点和本轮 7 个 exact route 均已有 resolver + hook tests。2026-05-12 已补 6 个 batch list hooks：`setting-manage/system-manage/change-password/list`、`setting-manage/system-manage/community-configuration/list`、`setting-manage/system-manage/register-protocol/list`、`dev-team/config-manage/type/list`、`dev-team/config-manage/item/list`、`dev-team/config-manage/center/list`。2026-05-16 已补 `setting-manage/system-manage/system-config/list`、`setting-manage/system-manage/initialize-cell/list`、`dev-team/config-manage/dictionary/list`；本轮已补 `operation-team` 13 个 list hook，以及 `expense-manage` A-E 共 14 个 list hook：`cancel-fee/list`、`contracte-charge/list`、`discount-apply/list`、`discount-setting/list`、`discount-type/list`、`expense-summary-table/list`、`meter-reading-type/list`、`overdue-payment-information/list`、`payment-review/list`、`refund-review/list`、`reminder-for-overdue-payments/list`、`reprint-voucher/list`、`vehicle-charge/list`、`water-and-electricity-meter-reading/list`。剩余接力重点是 `expense-summary-table` 的 report-manage / expense-manage 路径冲突，以及 `community-manage`、`dev-team/menu-manage` / `cache-manage` 等 resolver 缺口。
4. Chrome MCP 页面验证已覆盖 P1 report-manage 四端点；后续新增 endpoint 仍只有真实页面组件发出的 Network 请求才能补 `browserEvidence`。
5. 再按 Neon main 方案跑 `PHASE7_E2E_*` 写入-回读-清理或只读 DB 验收；本地 `__nitro/ready` 503 不能写成 `DB_READY`。用户已明确不要 Neon 测试分支。
