...
| 维度               | 当前口径                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Admin 旧 API       | 当前 working tree 口径：`apps/admin/server/api/**/*.ts` 共 155 个旧 API 文件；`apps/api/server/routes/api/**/*.ts` 共 159 个 server route；exact legacy path 未覆盖仅剩 1 个（`report-manage/expense-summary-table/list.post.ts` 路径冲突保留项）。2026-05-16 补齐 48 个 CRUD/边缘 route 后，admin 旧路径精确覆盖率达到 154/155。P1 四个 `report-manage` 端点已在 staged working tree 接线，并已补本地语义验收和 `/api-shadow` 页面 Network；但仍缺 Neon main `DB_READY`、真实库样本复核、shadow-off/fallback 页面演练，不能写成完成或旧服务可退役                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- [ ] 仍缺 Chrome MCP / 页面 Network 证据；当前只能标 `db-read-repository-wired`，不得写 `DB_READY`。
- [ ] 本批未涉及写入口，未触发 guard/read-back/rollback。
- [x] 2026-05-16 Admin 旧路径全覆盖批次完成：48 个未覆盖旧路径已全部在 `apps/api` 补齐 route 实现。`apps/api/server/routes/api/**/*.ts` 从 109 增长到 159 个。仅剩 `report-manage/expense-summary-table/list.post.ts` 一个已知路径冲突保留项。
...
当前 working tree 口径：`apps/api/server/routes/api/**/*.ts` 为 159 个 server route；`apps/admin/server/api/**/*.ts` 为 155 个 legacy file；exact legacy path 未覆盖仅剩 1 个（`report-manage/expense-summary-table/list.post.ts` 路径冲突保留项）。生产 readiness 仍只能写 `READY_CONFIGURED-only`，直到 `RUN_PHASE7_DB_READINESS_CHECK=1` 且 `/__nitro/ready` 返回 `DB_READY`。
...
1. **admin 旧路径精确覆盖已达 154/155**（2026-05-16）。`apps/api/server/routes/api/**/*.ts` 从 109 增长到 159 个。仅剩 `report-manage/expense-summary-table/list.post.ts` 一个已知路径冲突保留项。
2. **admin 前端 resolver 迁移已全部完成**（2026-05-16）。除 `report-manage/expense-summary-table` 路径冲突保留项外，`apps/admin/src/api/**/index.ts` 中不再有硬编码 `const API_URL = "/api/..."` 的 list hook。
...
4. `report-manage/expense-summary-table/index.ts` 保持硬编码：`apps/api` 只有 `expense-manage/expense-summary-table/list.post.ts`，无对应 `report-manage` 路由。该项需要路径映射方案或保持现状。
5. 下一步工作重点（按优先级）：
   - Chrome MCP 页面 Network 验证：本轮 48 个新增 route 对应的 admin 页面需要逐个验证 Network 请求命中 `/api-shadow` 并返回 200。
