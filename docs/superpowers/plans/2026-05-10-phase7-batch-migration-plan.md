...
- App legacy 21 endpoint 证据仍保持 Nitro manifest / allowlist / guard / fallback 口径，不能写成页面级 `browserEvidence` 或退役证据。
...
- 下一步：`report-manage/expense-summary-table/list` 与 P1 `report-manage` 剩余 7 页不再列为页面 Network / HTTP gate 待补；后续继续补其他未覆盖页面/端点、CRUD DB/R2、生产 `DB_READY`、App 端批次、真实库样本复核、shadow-off/fallback、必要写入回滚和独立复核证据。退役评估仍保持 no-go。
...
5. 下一步工作重点（按优先级）：
   - Chrome MCP 页面 Network / HTTP gate 补证：`property-manage/report-manage/expense-summary-table/list` 和 P1 `report-manage` 剩余 7 页已补页面级证据；其余新增 route 对应的 admin 页面或无页面入口端点仍需逐项补证。
