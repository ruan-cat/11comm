...
- [ ] 仍缺本轮 34 个 hook 对应页面的 Chrome MCP 页面级 Network 证据；当前只能标 hook-level resolver evidence，不得写成 `browserEvidence`、生产 `DB_READY` 或旧服务可退役。
...
- [ ] 下一轮继续处理：Chrome MCP 页面 Network 验证、`report-manage/expense-summary-table` 路径冲突最终决策、Neon main `PHASE7_E2E_*` 写入-回读-清理或只读 DB 验收、shadow-off/fallback 页面演练，最后才评估旧服务退役。
