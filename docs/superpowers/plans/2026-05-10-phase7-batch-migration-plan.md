...
| Admin 前端切流 | `report-manage` exact route hooks 已补：`payment-details-form`、上一轮 P1 四端点和本轮 7 个 exact route 均已使用 `resolveAdminApiRequestUrl` 并补 hook tests。2026-05-12 batch list resolver 本轮新增 6 个 admin hook resolver + hook tests：`setting-manage/system-manage/change-password/list`、`setting-manage/system-manage/community-configuration/list`、`setting-manage/system-manage/register-protocol/list`、`dev-team/config-manage/type/list`、`dev-team/config-manage/item/list`、`dev-team/config-manage/center/list`；主控复验 4 files / 18 tests passed。`report-manage/expense-summary-table/list` 因 `apps/api` 不存在对应 report-manage 路径、仅存在 `expense-manage/expense-summary-table/list.post.ts`，仍按路径冲突和硬编码保留，不写成已切流。剩余代表性 resolver 缺口仍包括 `dev-team/config-manage/dictionary/list`、`setting-manage/system-manage/system-config/list`、`setting-manage/system-manage/initialize-cell/list`、`operation-team/**`、`expense-manage`、`community-manage` |
...
- [ ] 下一轮优先补上述 4 端点的字段映射、过滤条件、JSONB 解析/聚合语义和 Vitest；P1 四端点与 `report-manage` exact route hooks 已补 `resolveAdminApiRequestUrl` 和 hook tests，后续继续处理 `expense-summary-table` 路径冲突和剩余 2026-05-12 batch list resolver 缺口（`dev-team/config-manage/dictionary/list`、`setting-manage/system-manage/system-config/list`、`setting-manage/system-manage/initialize-cell/list`、`operation-team/**`、`expense-manage`、`community-manage`）；再做 Chrome MCP 页面验证、Neon main `PHASE7_E2E_*` 写入-回读-清理或只读 DB 验收，最后才评估旧服务退役。
...
- 下一小批建议优先处理 `setting-manage/system-manage/system-config/list`、`setting-manage/system-manage/initialize-cell/list`、`dev-team/config-manage/dictionary/list`。这些仍属于 hook-level resolver 补齐，完成后必须立刻同步本计划、本矩阵、总设计文档和 Memorix。
...
3. `report-manage` exact route hooks 已补前端 API hooks 与 `resolveAdminApiRequestUrl`：`payment-details-form`、P1 四端点和本轮 7 个 exact route 均已有 resolver + hook tests。本轮另补 6 个 2026-05-12 batch list hooks：`setting-manage/system-manage/change-password/list`、`setting-manage/system-manage/community-configuration/list`、`setting-manage/system-manage/register-protocol/list`、`dev-team/config-manage/type/list`、`dev-team/config-manage/item/list`、`dev-team/config-manage/center/list`。剩余接力重点是 `expense-summary-table` 的 report-manage / expense-manage 路径冲突，以及 `dev-team/config-manage/dictionary/list`、`setting-manage/system-manage/system-config/list`、`setting-manage/system-manage/initialize-cell/list`、`operation-team/**`、`expense-manage`、`community-manage` 等 resolver 缺口。
