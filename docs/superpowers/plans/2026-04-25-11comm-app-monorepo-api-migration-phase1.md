...
| app `src/types` tracked source audit                                                    | PASS: 旧源 `git ls-files src/types` 与目标 `git ls-files apps/app/src/types` 均为 27 个 `.ts` 类型源文件；缺失 0、额外 0；CRLF 归一化后文本差异 0。旧源和目标的 `src/types/*.d.ts` 均为 ignored 生成文件，不作为应提交快照补全项。        |
