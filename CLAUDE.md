...
**重要规则：运行 `test:nitro` 测试命令前，必须先运行 `dev` 命令**

本项目的 Nitro 接口需要连接 Neon 数据库进行测试，因此在运行测试命令之前，必须先确保本地开发服务器正在运行。

```bash
# 步骤 1：首先启动开发服务器（提供本地 nitro 接口）
cd apps/admin
pnpm dev

# 步骤 2：然后在新的终端窗口运行测试命令
cd apps/admin
pnpm test:nitro
```
...
1. `test:nitro` 测试命令用于测试 Nitro 接口在连接 Neon 数据库的前提下是否能正常工作
2. 需要先启动 `dev` 命令，确保提供本地的、可运行的 nitro 接口
3. 测试命令会连接到 Neon 数据库进行真实的数据库操作验证
4. 正确的运行顺序可以确保测试结果准确反映生产环境的实际行为

**未来提醒：**

- 以后的 AI 模型都应该记得这个规定
- 每次修改 Nitro 接口或数据库相关代码后，应该运行 `test:nitro` 命令来验证接口是否能正常工作
- 如果测试失败，需要检查数据库连接配置和环境变量是否正确
